  ...
  const builtin_interfaces::msg::Time & stamp) const
{
  ...
  // Rasterize each edge corridor as free
  for (const auto & corridor : corridors) {
    rasterizeEdge(corridor, grid);
  }

  // Rasterize circular free area at each node (fills junction gaps)
  ...
  nav_msgs::msg::OccupancyGrid & grid) const
{
  ...
  // Normal vector (perpendicular, left side)
  double nx = -uy;
  double ny = ux;
  ...
  // Compute AABB of the rotated rectangle (4 corners)
  ...
  // Convert AABB to grid range (floor for min, ceil for max)
  ...
  // Check each cell in AABB: project onto edge axes
  for (int gy = gy_min; gy <= gy_max; ++gy) {
    for (int gx = gx_min; gx <= gx_max; ++gx) {
      ...
      // Vector from edge midpoint to cell center
      double vx = wx - cx;
      double vy = wy - cy;

      // Project onto edge direction (along)
      double proj_along = vx * ux + vy * uy;
      // Project onto edge normal (perpendicular)
      double proj_perp = vx * nx + vy * ny;

      // Inside rotated rectangle?
      if (std::abs(proj_along) <= half_length &&
          std::abs(proj_perp) <= half_width)
      {
        int index = gy * grid_cols_ + gx;
        grid.data[index] = params_.free_value;
      }
    }
  }
}
...
{
  if (edge_path_width > 0.0) {
    return edge_path_width;
  }
  return params_.default_path_width;
}
